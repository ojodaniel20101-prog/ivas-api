...
def login():
    ...
    # Try multiple Chrome profiles to find one Cloudflare accepts
    profiles = ['chrome136', 'chrome120', 'chrome110', 'chrome101', 'chrome99', 'safari15_5']
    s = None
    r = None
    for profile in profiles:
        try:
            ...
            logger.info(f'Profile {profile} -> {r.status_code}')
            if r.status_code == 200:
                logger.info(f'✅ Cloudflare bypassed with: {profile}')
                break
        except Exception as e:
            logger.info(f'Profile {profile} error: {e}')
            continue
    else:
        raise Exception('All profiles blocked by Cloudflare')

    if not r or r.status_code != 200:
        raise Exception(f'Login page returned {r.status_code if r else "no response"}')
...
@app.route('/test')
def test():
    """Test if curl_cffi can reach ivasms at all"""
    from curl_cffi import requests as cf
    results = {}
    for profile in ['chrome136', 'chrome120', 'chrome110', 'chrome101']:
        try:
            s = cf.Session(impersonate=profile)
            r = s.get(LOGIN_URL, headers=HEADERS, timeout=15)
            results[profile] = r.status_code
        except Exception as e:
            results[profile] = str(e)[:50]
